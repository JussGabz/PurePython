...
def new_workout_plan():
    new_workout_plan = WorkoutPlan(name="New Workout Plan", exercises=[], difficulty="Hard", date_created="Today")
...
def test_workoutplan_exercises(new_workout_plan):
    assert new_workout_plan.get_exercises() == []

def test_workoutplan_creation_date(new_workout_plan):
    assert new_workout_plan.get_date_created() == "Today"

def test_add_exercise(new_workout_plan):
    new_workout_plan.add_exercise(new_exercise)
    assert new_workout_plan.exercises == [new_exercise]

def test_add_exercise_existing(new_workout_plan):
    new_workout_plan.add_exercise(new_exercise)
    new_workout_plan.add_exercise(new_exercise)
    assert new_workout_plan.get_exercises() == [new_exercise, new_exercise]